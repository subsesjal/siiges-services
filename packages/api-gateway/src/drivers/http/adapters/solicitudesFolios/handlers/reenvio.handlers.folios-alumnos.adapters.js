 ...
 */
async function sendNotificationReport(processor, tipoNotificacion, solicitudFolio) {
  const action = NOTIFICATION_MAPPING[tipoNotificacion];
...
async function envioNotificacion(req, reply) {
  try {
    ...
    await sendNotificationReport(processor, tipoNotificacion, solicitudFolio);
