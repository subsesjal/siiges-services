...
async function findGroupUsuariosUsuarios(fastify, identifier, query) {
  Logger.info('[api/usuarios/findGroup]: finding the list of usuarios - usuarios');

  const usuarios = await fastify.usuarioServices.findAllUserUsers(identifier, query);
...
async function findGroup(req, reply) {
  try {
    ...
    const usuarios = await findGroupUsuariosUsuarios(this, usuarioId, req.query);
